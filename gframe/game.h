...
struct info_panel_elements {
  ...
  // infos
  int infosExpanded; // 0: not expanded, 1: expanded and to be shown as
                     // expanded, 2: expanded but not to be shown as expanded
...
struct host_creation_panel_elements {
  ...
  irr::gui::IGUICheckBox *chkCustomRules[7 + 12 + 8 + 5];
...
class Game final : public info_panel_elements,
                   public main_menu_panel_elements,
                   public lan_panel_elements,
                   public host_creation_panel_elements,
                   public host_panel_elements,
                   public replay_panel_elements,
                   public puzzle_panel_elements,
                   public deck_edit_page_elements,
                   public server_lobby_page_elements,
                   public game_field_elements {
...
public:
...
#endif
  bool MainLoop();
  bool ApplySkin(const epro::path_string &skin, bool reload = false,
                 bool firstrun = false);
  ...
  void DrawSelectionLine(const Materials::QuadVertex vec, bool strip, int width,
  ...
  void DrawStackIndicator(epro::wstringview text, const Materials::QuadVertex v,
                          bool opponent);
  void DrawGUI();
  void DrawTextureBilinear(irr::video::ITexture *texture,
  ...
  void DrawThumb(const CardDataC *cp, irr::core::vector2di pos, LFList *lflist,
                 bool drag = false, const irr::core::recti *cliprect = nullptr,
                 bool loadimage = true);
  void DrawDeckBd();
  void SaveConfig();
  struct RepoGui {
  ...
  void LoadServers();
  void ShowCardInfo(uint32_t code, bool resize = false,
                    imgType type = imgType::ART);
  ...
  void PopupSaveWindow(epro::wstringview caption, epro::wstringview text,
  ...
  void OnResize();
  template <typename T> T Scale(T val) const;
  ...
  template <typename T> T ResizeX(T x) const;
  template <typename T> T ResizeY(T y) const;
  template <typename T, typename T2>
  irr::core::vector2d<T> Scale(T x, T2 y) const;
  irr::core::recti Resize(irr::s32 x, irr::s32 y, irr::s32 x2,
                          irr::s32 y2) const;
  irr::core::recti Resize(irr::s32 x, irr::s32 y, irr::s32 x2, irr::s32 y2,
                          irr::s32 dx, irr::s32 dy, irr::s32 dx2,
                          irr::s32 dy2) const;
  irr::core::vector2d<irr::s32> Resize(irr::s32 x, irr::s32 y,
                                       bool reverse = false) const;
  irr::core::recti ResizeWithCappedWidth(irr::s32 x, irr::s32 y, irr::s32 x2,
                                         irr::s32 y2, float targetAspectRatio,
                                         bool scale = true) const;
  irr::core::recti ResizeElem(irr::s32 x, irr::s32 y, irr::s32 x2, irr::s32 y2,
                              bool scale = true) const;
  irr::core::recti ResizePhaseHint(irr::s32 x, irr::s32 y, irr::s32 x2,
                                   irr::s32 y2, irr::s32 width) const;
  irr::core::recti ResizeWinFromCenter(irr::s32 x, irr::s32 y, irr::s32 x2,
                                       irr::s32 y2, irr::s32 xoff = 0,
                                       irr::s32 yoff = 0) const;
  irr::core::recti ResizeWin(irr::s32 x, irr::s32 y, irr::s32 x2, irr::s32 y2,
                             bool chat = false) const;
  ...
  epro::path_string FindScript(epro::path_stringview script_name,
                               irr::io::IReadFile **retarchive = nullptr);
  std::vector<char> FindAndReadScript(epro::stringview script_name);
  std::vector<char> ReadScript(epro::path_stringview script_name,
                               irr::io::IReadFile *archive = nullptr);
  ...
  static void UpdateDownloadBar(int percentage, int cur, int tot,
                                const char *filename, bool is_new,
                                void *payload);
  ...
  ModularCardRenderer *modularRenderer;
  ModularArtManager *modularArtManager;
  irr::video::ITexture *modularCardTexture; // Rendered card for bilinear draw
#ifdef YGOPRO_BUILD_DLL
  void *ocgcore;
  bool coreJustLoaded;
#endif
  ...
  using locale_entry_t =
      std::pair<epro::path_string, std::vector<epro::path_string>>;
...
template <typename T>
inline irr::core::vector2d<T>
Game::Scale(const irr::core::vector2d<T> &vec) const {
  return Scale<T>(vec.X, vec.Y);
}
template <typename T> inline T Game::ResizeX(T x) const {
  return Scale<T>(x * window_scale.X);
}
template <typename T> inline T Game::ResizeY(T y) const {
...
template <typename T> inline T Game::Scale(T val) const {
  return T(val * dpi_scale);
}
template <typename T, typename T2, typename T3, typename T4>
inline irr::core::rect<T> Game::Scale(T x, T2 y, T3 x2, T4 y2) const {
  const auto &scale = dpi_scale;
  return {(T)std::roundf(x * scale), (T)std::roundf(y * scale),
          (T)std::roundf(x2 * scale), (T)std::roundf(y2 * scale)};
}
template <typename T>
inline irr::core::rect<T> Game::Scale(const irr::core::rect<T> &rect) const {
  return Scale(rect.UpperLeftCorner.X, rect.UpperLeftCorner.Y,
