...
// Draw texture with bilinear filtering for smooth scaling
void Game::DrawTextureBilinear(irr::video::ITexture *texture,
  ...
  // Calculate screen-space coordinates (-1 to 1 range)
  float screenWidth = (float)driver->getScreenSize().Width;
  float screenHeight = (float)driver->getScreenSize().Height;

  float x1 = ((float)destRect.UpperLeftCorner.X / screenWidth - 0.5f) * 2.0f;
  float y1 = ((float)destRect.UpperLeftCorner.Y / screenHeight - 0.5f) * -2.0f;
  float x2 = ((float)destRect.LowerRightCorner.X / screenWidth - 0.5f) * 2.0f;
  float y2 = ((float)destRect.LowerRightCorner.Y / screenHeight - 0.5f) * -2.0f;

  // Create vertices for a quad
  irr::video::S3DVertex vertices[4];
  vertices[0] = irr::video::S3DVertex(
      x1, y1, 0, 0, 0, 1, irr::video::SColor(255, 255, 255, 255), 0.0f, 0.0f);
  vertices[1] = irr::video::S3DVertex(
      x2, y1, 0, 0, 0, 1, irr::video::SColor(255, 255, 255, 255), 1.0f, 0.0f);
  vertices[2] = irr::video::S3DVertex(
      x1, y2, 0, 0, 0, 1, irr::video::SColor(255, 255, 255, 255), 0.0f, 1.0f);
  vertices[3] = irr::video::S3DVertex(
      x2, y2, 0, 0, 0, 1, irr::video::SColor(255, 255, 255, 255), 1.0f, 1.0f);

  irr::u16 indices[6] = {0, 1, 2, 3, 2, 1};

  // Set up material with bilinear filtering
  ...
  material.TextureLayer[0].AnisotropicFilter = 4;
...
void Game::DrawSelectionLine(const Materials::QuadVertex vec, bool strip,
                             int width, irr::video::SColor color) {
  ...
  if (strip && !gGameConfig->dotted_lines) {
    ...
    auto drawLine = [&](const auto &pos1, const auto &pos2) -> void {
      if (swap)
        driver->draw3DLineW(pos1 + (pos2 - pos1) * (pattern) / 15.0, pos2,
                            color, width);
      else
        driver->draw3DLineW(pos1, pos1 + (pos2 - pos1) * (pattern) / 15.0,
                            color, width);
    };
    ...
    const std::array<irr::core::vector3df, 4> pos{vec[0].Pos, vec[1].Pos,
                                                  vec[3].Pos, vec[2].Pos};
    driver->draw3DShapeW(pos.data(), static_cast<irr::u32>(pos.size()), color,
                         width, strip ? linePatternGL : 0xffff);
  }
}
void Game::DrawBackGround() {
  ...
  auto DrawTextureRect = [this](Materials::QuadVertex vertices,
  ...
  // draw field
  DrawTextureRect(matManager.vField,
                  DrawFieldSpell()
  ...
  // select field
  if ((dInfo.curMsg == MSG_SELECT_PLACE ||
       dInfo.curMsg == MSG_SELECT_DISFIELD || dInfo.curMsg == MSG_HINT) &&
      dField.selectable_field) {
    irr::video::SColor outline_color =
        skin::DUELFIELD_SELECTABLE_FIELD_OUTLINE_VAL;
    uint32_t filter = 0x1;
    for (int i = 0; i < 7; ++i, filter <<= 1) {
      if (dField.selectable_field & filter)
        DrawSelectionLine(matManager.vFieldMzone[0][i],
                          !(dField.selected_field & filter), 2, outline_color);
    }
    filter = 0x100;
    for (int i = 0; i < 8; ++i, filter <<= 1) {
      if (dField.selectable_field & filter)
        DrawSelectionLine(matManager.getSzone()[0][i],
                          !(dField.selected_field & filter), 2, outline_color);
    }
    filter = 0x10000;
    for (int i = 0; i < 7; ++i, filter <<= 1) {
      if (dField.selectable_field & filter)
        DrawSelectionLine(matManager.vFieldMzone[1][i],
                          !(dField.selected_field & filter), 2, outline_color);
    }
    filter = 0x1000000;
    for (int i = 0; i < 8; ++i, filter <<= 1) {
      if (dField.selectable_field & filter)
        DrawSelectionLine(matManager.getSzone()[1][i],
                          !(dField.selected_field & filter), 2, outline_color);
    }
  }
  // disabled field
  {
    irr::video::SColor disabled_color =
        skin::DUELFIELD_DISABLED_FIELD_COLOR_VAL;
    uint32_t filter = 0x1;
    for (int i = 0; i < 7; ++i, filter <<= 1) {
      if (dField.disabled_field & filter) {
        driver->draw3DLine(matManager.vFieldMzone[0][i][0].Pos,
                           matManager.vFieldMzone[0][i][3].Pos, disabled_color);
        driver->draw3DLine(matManager.vFieldMzone[0][i][1].Pos,
                           matManager.vFieldMzone[0][i][2].Pos, disabled_color);
      }
    }
    filter = 0x100;
    for (int i = 0; i < 8; ++i, filter <<= 1) {
      if (dField.disabled_field & filter) {
        driver->draw3DLine(matManager.getSzone()[0][i][0].Pos,
                           matManager.getSzone()[0][i][3].Pos, disabled_color);
        driver->draw3DLine(matManager.getSzone()[0][i][1].Pos,
                           matManager.getSzone()[0][i][2].Pos, disabled_color);
      }
    }
    filter = 0x10000;
    for (int i = 0; i < 7; ++i, filter <<= 1) {
      if (dField.disabled_field & filter) {
        driver->draw3DLine(matManager.vFieldMzone[1][i][0].Pos,
                           matManager.vFieldMzone[1][i][3].Pos, disabled_color);
        driver->draw3DLine(matManager.vFieldMzone[1][i][1].Pos,
                           matManager.vFieldMzone[1][i][2].Pos, disabled_color);
      }
    }
    filter = 0x1000000;
    for (int i = 0; i < 8; ++i, filter <<= 1) {
      if (dField.disabled_field & filter) {
        driver->draw3DLine(matManager.getSzone()[1][i][0].Pos,
                           matManager.getSzone()[1][i][3].Pos, disabled_color);
        driver->draw3DLine(matManager.getSzone()[1][i][1].Pos,
                           matManager.getSzone()[1][i][2].Pos, disabled_color);
      }
    }
  }
  auto setAlpha = [](irr::video::SMaterial &material,
                     const irr::video::SColor &color) {
    uint32_t endalpha =
        std::round(color.getAlpha() * (selFieldAlpha - 5.0) * (0.005));
    material.DiffuseColor = endalpha << 24;
    material.AmbientColor = color;
  };
  // current sel
  if (dField.hovered_location == 0 ||
      dField.hovered_location == LOCATION_HAND ||
      dField.hovered_location == POSITION_HINT)
    return;
  if (!dInfo.HasFieldFlag(DUEL_EMZONE) &&
      dField.hovered_location == LOCATION_MZONE && dField.hovered_sequence > 4)
    return;
  if ((!dInfo.HasFieldFlag(DUEL_SEPARATE_PZONE) &&
       dField.hovered_location == LOCATION_SZONE &&
       dField.hovered_sequence > 5))
    return;
  ...
  if (dField.hovered_location == LOCATION_DECK)
    ...
    vertex =
        matManager
            .vFieldMzone[dField.hovered_controler][dField.hovered_sequence];
    ClientCard *pcard =
    ...
    vertex = matManager
                 .getSzone()[dField.hovered_controler][dField.hovered_sequence];
    ClientCard *pcard =
        dField.szone[dField.hovered_controler][dField.hovered_sequence];
    if (pcard && (pcard->type & TYPE_LINK) && (pcard->position & POS_FACEUP))
      DrawLinkedZones(pcard);
...
void Game::DrawLinkedZones(ClientCard *pcard) {
  ...
  if (dField.hovered_location == LOCATION_SZONE) {
    ...
    if (mark & LINK_MARKER_TOP_LEFT &&
        dField.hovered_sequence > (0 + three_columns)) {
      pcard2 =
          dField.mzone[dField.hovered_controler][dField.hovered_sequence - 1];
      CheckMutual(pcard2, LINK_MARKER_BOTTOM_RIGHT);
      driver->drawVertexPrimitiveList(
          &matManager.vFieldMzone[dField.hovered_controler]
                                 [dField.hovered_sequence - 1],
          4, matManager.iRectangle, 2);
    }
    ...
    if (mark & LINK_MARKER_TOP_RIGHT &&
        dField.hovered_sequence < (4 - three_columns)) {
      pcard2 =
          dField.mzone[dField.hovered_controler][dField.hovered_sequence + 1];
    ...
    if (mark & LINK_MARKER_LEFT &&
        dField.hovered_sequence > (0 + three_columns)) {
      pcard2 =
          dField.szone[dField.hovered_controler][dField.hovered_sequence - 1];
      CheckMutual(pcard2, LINK_MARKER_RIGHT);
      driver->drawVertexPrimitiveList(
          &matManager.getSzone()[dField.hovered_controler]
                                [dField.hovered_sequence - 1],
          4, matManager.iRectangle, 2);
    }
    if (mark & LINK_MARKER_RIGHT &&
        dField.hovered_sequence < (4 - three_columns)) {
      pcard2 =
          dField.szone[dField.hovered_controler][dField.hovered_sequence + 1];
  ...
  if (dField.hovered_sequence < 5) {
    ...
    if (dInfo.HasFieldFlag(DUEL_EMZONE)) {
      if ((mark & LINK_MARKER_TOP_LEFT && dField.hovered_sequence == 2) ||
        ...
        int other_mark =
            (dField.hovered_sequence == 2)   ? LINK_MARKER_BOTTOM_RIGHT
            : (dField.hovered_sequence == 1) ? LINK_MARKER_BOTTOM
                                             : LINK_MARKER_BOTTOM_LEFT;
        pcard2 = dField.mzone[dField.hovered_controler][5];
        if (!pcard2) {
          pcard2 = dField.mzone[1 - dField.hovered_controler][6];
          other_mark = (dField.hovered_sequence == 2)   ? LINK_MARKER_TOP_LEFT
                       : (dField.hovered_sequence == 1) ? LINK_MARKER_TOP
                                                        : LINK_MARKER_TOP_RIGHT;
        }
        CheckMutual(pcard2, other_mark);
        driver->drawVertexPrimitiveList(
            &matManager.vFieldMzone[dField.hovered_controler][5], 4,
            matManager.iRectangle, 2);
      }
      if ((mark & LINK_MARKER_TOP_LEFT && dField.hovered_sequence == 4) ||
        ...
        int other_mark =
            (dField.hovered_sequence == 4)   ? LINK_MARKER_BOTTOM_RIGHT
            : (dField.hovered_sequence == 3) ? LINK_MARKER_BOTTOM
                                             : LINK_MARKER_BOTTOM_LEFT;
        pcard2 = dField.mzone[dField.hovered_controler][6];
        if (!pcard2) {
          pcard2 = dField.mzone[1 - dField.hovered_controler][5];
          other_mark = (dField.hovered_sequence == 4)   ? LINK_MARKER_TOP_LEFT
                       : (dField.hovered_sequence == 3) ? LINK_MARKER_TOP
                                                        : LINK_MARKER_TOP_RIGHT;
        }
        CheckMutual(pcard2, other_mark);
        driver->drawVertexPrimitiveList(
            &matManager.vFieldMzone[dField.hovered_controler][6], 4,
            matManager.iRectangle, 2);
      }
    }
  } else {
    ...
    if (mark & LINK_MARKER_BOTTOM_LEFT && !(three_columns && swap == 0)) {
      ...
      driver->drawVertexPrimitiveList(
          &matManager.vFieldMzone[dField.hovered_controler][0 + swap], 4,
          matManager.iRectangle, 2);
    }
    if (mark & LINK_MARKER_BOTTOM) {
      ...
      driver->drawVertexPrimitiveList(
          &matManager.vFieldMzone[dField.hovered_controler][1 + swap], 4,
          matManager.iRectangle, 2);
    }
    if (mark & LINK_MARKER_BOTTOM_RIGHT && !(three_columns && swap == 2)) {
      ...
      driver->drawVertexPrimitiveList(
          &matManager.vFieldMzone[dField.hovered_controler][2 + swap], 4,
          matManager.iRectangle, 2);
    }
    if (mark & LINK_MARKER_TOP_LEFT && !(three_columns && swap == 0)) {
      ...
      CheckMutual(pcard2, LINK_MARKER_TOP_LEFT);
      driver->drawVertexPrimitiveList(
          &matManager.vFieldMzone[1 - dField.hovered_controler][4 - swap], 4,
          matManager.iRectangle, 2);
    }
    if (mark & LINK_MARKER_TOP) {
      ...
      CheckMutual(pcard2, LINK_MARKER_TOP);
      driver->drawVertexPrimitiveList(
          &matManager.vFieldMzone[1 - dField.hovered_controler][3 - swap], 4,
          matManager.iRectangle, 2);
    }
    if (mark & LINK_MARKER_TOP_RIGHT && !(three_columns && swap == 2)) {
      ...
      CheckMutual(pcard2, LINK_MARKER_TOP_RIGHT);
      driver->drawVertexPrimitiveList(
          &matManager.vFieldMzone[1 - dField.hovered_controler][2 - swap], 4,
          matManager.iRectangle, 2);
    }
  }
}
...
void Game::DrawCard(ClientCard *pcard) {
  ...
  matManager.mCard.DiffuseColor =
      ((int)std::round(pcard->curAlpha) << 24) | 0xffffff;
  driver->setTransform(irr::video::ETS_WORLD, pcard->mTransform);
  auto m22 = pcard->mTransform(2, 2);
  if (m22 > -0.99 || pcard->is_moving) {
    // Gameboard cards always use legacy rendering
    matManager.mCard.setTexture(
        0, imageManager.GetTextureCard(pcard->code, imgType::ART));
    driver->setMaterial(matManager.mCard);
    driver->drawVertexPrimitiveList(matManager.vCardFront, 4,
                                    matManager.iRectangle, 2);
  }
  if (m22 < 0.99 || pcard->is_moving) {
    ...
    if (txt == imageManager.tCover[0]) {
      ...
      matManager.mCard.setTexture(
          0, imageManager.GetTextureCard(pcard->cover, imgType::COVER));
    }
    driver->setMaterial(matManager.mCard);
    driver->drawVertexPrimitiveList(matManager.vCardBack, 4,
                                    matManager.iRectangle, 2);
  }
  if (pcard->is_selectable && (pcard->location & 0xe)) {
    irr::video::SColor outline_color =
        skin::DUELFIELD_SELECTABLE_CARD_OUTLINE_VAL;
    if ((pcard->location == LOCATION_HAND && pcard->code) ||
        ((pcard->location & 0xc) && (pcard->position & POS_FACEUP)))
      DrawSelectionLine(matManager.vCardOutline, !pcard->is_selected, 2,
                        outline_color);
    else
      DrawSelectionLine(matManager.vCardOutliner, !pcard->is_selected, 2,
                        outline_color);
  }
  if (pcard->is_highlighting) {
    irr::video::SColor outline_color =
        skin::DUELFIELD_HIGHLIGHTING_CARD_OUTLINE_VAL;
    if ((pcard->location == LOCATION_HAND && pcard->code) ||
        ((pcard->location & 0xc) && (pcard->position & POS_FACEUP)))
  ...
  if (pcard->is_showequip) {
    ...
    driver->setMaterial(matManager.mTexture);
    driver->drawVertexPrimitiveList(matManager.vSymbol, 4,
                                    matManager.iRectangle, 2);
    ...
    driver->drawVertexPrimitiveList(matManager.vSymbol, 4,
             ...
             (pcard->location & LOCATION_ONFIELD) &&
             (pcard->position & POS_FACEUP)) {
    matManager.mTexture.setTexture(0, imageManager.tNegated);
    driver->setMaterial(matManager.mTexture);
    driver->drawVertexPrimitiveList(matManager.vNegate, 4,
                                    matManager.iRectangle, 2);
  }
  if (pcard->is_moving)
    return;
  if (pcard->cmdFlag & COMMAND_ATTACK) {
    ...
    atk.setTranslation(pcard->curPos +
                       irr::core::vector3df(0,
                                            (pcard->controler == 0 ? -1 : 1) *
                                                (atkdy / 4.0f + 0.35f),
                                            0.05f));
    atk.setRotationRadians(
        irr::core::vector3df(0, 0, pcard->controler == 0 ? 0 : irr::core::PI));
...
template <typename T>
inline void DrawShadowTextPos(irr::gui::CGUITTFont *font, const T &text,
                              const irr::core::recti &shadowposition,
                              const irr::core::recti &mainposition,
                              irr::video::SColor color = 0xffffffff,
                              irr::video::SColor shadowcolor = 0xff000000,
                              bool hcenter = false, bool vcenter = false,
                              const irr::core::recti *clip = nullptr) {
...
ForceInline void DrawShadowText(irr::gui::CGUITTFont *font, const T &text,
                                const irr::core::recti &shadowposition,
                                const irr::core::recti &padding,
                                Args &&...args) {
  const irr::core::recti position(
      shadowposition.UpperLeftCorner.X + padding.UpperLeftCorner.X,
      shadowposition.UpperLeftCorner.Y + padding.UpperLeftCorner.Y,
      shadowposition.LowerRightCorner.X + padding.LowerRightCorner.X,
      shadowposition.LowerRightCorner.Y + padding.LowerRightCorner.Y);
  DrawShadowTextPos(font, text, shadowposition, position,
                    std::forward<Args>(args)...);
}
void Game::DrawMisc() {
  const float twoPI = 2.0f * irr::core::PI;
  static float act_rot = 0.0f;
  // pre expanded version of setRotationRadians, we're only setting the z value,
  // saves computations
  ...
  if (act_rot >= twoPI) {
    act_rot -= twoPI;
    // double branch to account for random instances where the value increases
    // too much
    if (act_rot >= twoPI) {
  ...
  auto drawact = [this, &im](const Materials::QuadVertex vertex, float zval) {
    im.setTranslation(
        irr::core::vector3df((vertex[0].Pos.X + vertex[1].Pos.X) / 2,
                             (vertex[0].Pos.Y + vertex[2].Pos.Y) / 2, zval));
    driver->setTransform(irr::video::ETS_WORLD, im);
    driver->drawVertexPrimitiveList(matManager.vActivate, 4,
                                    matManager.iRectangle, 2);
  };
  ...
  for (int p = 0; p < 2; p++) {
    ...
    if (dField.remove_act[p])
      drawact(matManager.getRemove()[p],
              dField.remove[p].size() * 0.01f + 0.02f);
  ...
  if (dField.conti_act) {
    im.setTranslation(
        irr::core::vector3df((matManager.vFieldContiAct[three_columns][0].X +
                              matManager.vFieldContiAct[three_columns][1].X) /
                                 2,
                             (matManager.vFieldContiAct[three_columns][0].Y +
                              matManager.vFieldContiAct[three_columns][2].Y) /
                                 2,
                             0.03f));
    driver->setTransform(irr::video::ETS_WORLD, im);
    driver->drawVertexPrimitiveList(matManager.vActivate, 4,
  ...
  auto setCoords = [&](size_t i) {
    const auto div = i / 5;
    const auto mod = i % 5;
    matManager.vChainNum[0].TCoords =
        irr::core::vector2df(0.19375f * mod, 0.2421875f * div);
    matManager.vChainNum[1].TCoords =
        irr::core::vector2df(0.19375f * (mod + 1), 0.2421875f * div);
    matManager.vChainNum[2].TCoords =
        irr::core::vector2df(0.19375f * mod, 0.2421875f * (div + 1));
    matManager.vChainNum[3].TCoords =
        irr::core::vector2df(0.19375f * (mod + 1), 0.2421875f * (div + 1));
  };
  for (size_t i = 0; i < dField.chains.size(); ++i) {
    ...
    driver->drawVertexPrimitiveList(matManager.vChainNum, 4,
                                    matManager.iRectangle, 2);
  }
  // lp bar
  ...
  const auto lpframe_pos =
      ((dInfo.turn % 2 && dInfo.isFirst) ||
       (!(dInfo.turn % 2) && !dInfo.isFirst))
          ? Resize(327, 8, 630,
                   51 + static_cast<irr::s32>(23 * (self.size() - 1)))
          : Resize(689, 8, 991,
                   51 + static_cast<irr::s32>(23 * (oppo.size() - 1)));
  driver->draw2DRectangle(skin::DUELFIELD_TURNPLAYER_COLOR_VAL, lpframe_pos);
  driver->draw2DRectangleOutline(lpframe_pos,
                                 skin::DUELFIELD_TURNPLAYER_OUTLINE_COLOR_VAL);
  driver->draw2DImage(imageManager.tLPFrame, Resize(330, 10, 629, 30),
                      irr::core::recti(0, 0, 200, 20), 0, 0, true);
  driver->draw2DImage(imageManager.tLPFrame, Resize(691, 10, 990, 30),
                      irr::core::recti(0, 0, 200, 20), 0, 0, true);

#define SKCOLOR(what) skin::LPBAR_##what##_VAL
#define RECTCOLOR(what)                                                        \
  SKCOLOR(what##_TOP_LEFT), SKCOLOR(what##_TOP_RIGHT),                         \
      SKCOLOR(what##_BOTTOM_LEFT), SKCOLOR(what##_BOTTOM_RIGHT)
#define DRAWRECT(rect_pos, what, clip)                                         \
  do {                                                                         \
    driver->draw2DRectangleClip(rect_pos, RECTCOLOR(what), nullptr, clip);     \
  } while (0)
  if (dInfo.lp[0]) {
    const auto lpbar_pos = Resize(335, 12, 625, 28);
    if (dInfo.lp[0] < dInfo.startlp) {
      auto cliprect = Resize(
          335, 12,
          335 + 290 * (dInfo.lp[0] / static_cast<double>(dInfo.startlp)), 28);
  ...
  if (dInfo.lp[1] > 0) {
    ...
    if (dInfo.lp[1] < dInfo.startlp) {
      auto cliprect =
          Resize(986 - 290 * (dInfo.lp[1] / static_cast<double>(dInfo.startlp)),
                 12, 986, 28);
  ...
  if (lpcstring.size()) {
    if (lpplayer == 0)
      DrawShadowText(lpcFont, lpcstring, Resize(400, 470, 920, 520),
                     Resize(0, 2, 2, 0), (lpcalpha << 24) | lpccolor,
                     (lpcalpha << 24) | 0x00ffffff, true);
    else
      DrawShadowText(lpcFont, lpcstring, Resize(400, 160, 920, 210),
                     Resize(0, 2, 2, 0), (lpcalpha << 24) | lpccolor,
                     (lpcalpha << 24) | 0x00ffffff, true);
  }
...
#define SKCOLOR(what) skin::TIMEBAR_##what##_VAL
  ...
  if (!dInfo.isReplay && !dInfo.isSingleMode && dInfo.player_type < 7 &&
      dInfo.time_limit) {
    auto rectpos = Resize(525, 34, 625, 44);
    auto cliprect =
        Resize(525, 34, 525 + dInfo.time_left[0] * 100 / dInfo.time_limit, 44);
    DRAWRECT(rectpos, 1, &cliprect);
    driver->draw2DRectangleOutline(rectpos, skin::TIMEBAR_1_OUTLINE_VAL);
    rectpos = Resize(695, 34, 795, 44);
    cliprect =
        Resize(795 - dInfo.time_left[1] * 100 / dInfo.time_limit, 34, 795, 44);
    DRAWRECT(rectpos, 2, &cliprect);
    driver->draw2DRectangleOutline(rectpos, skin::TIMEBAR_2_OUTLINE_VAL);
  }

  DrawShadowText(numFont, dInfo.strLP[0], Resize(330, 11, 629, 29),
                 Resize(0, 1, 2, 0), skin::DUELFIELD_LP_1_VAL, 0xff000000, true,
                 true);
  DrawShadowText(numFont, dInfo.strLP[1], Resize(691, 11, 990, 29),
                 Resize(0, 1, 2, 0), skin::DUELFIELD_LP_2_VAL, 0xff000000, true,
                 true);
  ...
  /*driver->draw2DRectangle(Resize(632, 10, 688, 30), 0x00000000, 0x00000000,
  0xffffffff, 0xffffffff); driver->draw2DRectangle(Resize(632, 30, 688, 50),
  0xffffffff, 0xffffffff, 0x00000000, 0x00000000);*/
  DrawShadowText(lpcFont, gDataManager->GetNumString(dInfo.turn),
                 Resize(635, 5, 685, 40), Resize(0, 0, 2, 0),
                 skin::DUELFIELD_TURN_COUNT_VAL, 0x80000000, true);
#undef DRAWRECT
#undef LPCOLOR
#undef SKCOLOR
  ...
  for (size_t p = 0; p < 2; ++p) {
    ...
    if (dField.extra[p].size()) {
      const auto str = (dField.extra_p_count[p])
                           ? epro::format(L"{}({})", dField.extra[p].size(),
                                          dField.extra_p_count[p])
                           : epro::format(L"{}", dField.extra[p].size());
      DrawStackIndicator(str, matManager.getExtra()[p], (p == 1));
    }
    if (dField.deck[p].size())
      DrawStackIndicator(gDataManager->GetNumString(dField.deck[p].size()),
                         matManager.getDeck()[p], (p == 1));
    if (dField.grave[p].size())
      DrawStackIndicator(gDataManager->GetNumString(dField.grave[p].size()),
                         matManager.getGrave()[p], (p == 1));
    if (dField.remove[p].size())
      DrawStackIndicator(gDataManager->GetNumString(dField.remove[p].size()),
                         matManager.getRemove()[p], (p == 1));
  }
}
/*
...
void Game::DrawStatus(ClientCard *pcard) {
  auto getcoords = [collisionmanager =
                        device->getSceneManager()->getSceneCollisionManager()](
                       const irr::core::vector3df &pos3d) {
    return collisionmanager->getScreenCoordinatesFrom3DPosition(pos3d);
  };
  int x1, y1, x2, y2;
  if (pcard->controler == 0) {
    auto coords = getcoords(
        {pcard->curPos.X, (0.39f + pcard->curPos.Y), pcard->curPos.Z});
    x1 = coords.X;
    y1 = coords.Y;
    coords = getcoords({(pcard->curPos.X - 0.48f), (pcard->curPos.Y - 0.66f),
                        pcard->curPos.Z});
    x2 = coords.X;
    y2 = coords.Y;
  } else {
    auto coords = getcoords(
        {pcard->curPos.X, (pcard->curPos.Y - 0.66f), pcard->curPos.Z});
    x1 = coords.X;
    y1 = coords.Y;
    coords = getcoords({(pcard->curPos.X - 0.48f), (0.39f + pcard->curPos.Y),
                        pcard->curPos.Z});
    x2 = coords.X;
    y2 = coords.Y;
  }
  ...
  if (pcard->type & TYPE_LINK) {
    DrawShadowText(adFont, pcard->atkstring,
                   irr::core::recti(x1 - std::floor(atk.Width / 2), y1,
                                    x1 + std::floor(atk.Width / 2), y1 + 1),
                   padding_1111, GetAtkColor(), 0xff000000, true);
  } else {
    DrawShadowText(adFont, L"/",
                   irr::core::recti(x1 - half_slash_width, y1,
                                    x1 + half_slash_width, y1 + 1),
                   padding_1111, 0xffffffff, 0xff000000, true);
    DrawShadowText(
        adFont, pcard->atkstring,
        irr::core::recti(x1 - half_slash_width - atk.Width - slash.Width, y1,
                         x1 - half_slash_width, y1 + 1),
        padding_1111, GetAtkColor(), 0xff000000);
    DrawShadowText(adFont, pcard->defstring,
                   irr::core::recti(x1 + half_slash_width + slash.Width, y1,
                                    x1 - half_slash_width, y1 + 1),
  ...
  if (pcard->level != 0 && pcard->rank != 0) {
    DrawShadowText(adFont, L"/",
                   irr::core::recti(x2 - half_slash_width, y2,
                                    x2 + half_slash_width, y2 + 1),
                   padding_1111, 0xffffffff, 0xff000000, true);
    DrawShadowText(
        adFont, pcard->lvstring,
        irr::core::recti(x2 - half_slash_width - atk.Width - slash.Width, y2,
                         x2 - half_slash_width, y2 + 1),
        padding_1111, GetLevelColor(), 0xff000000);
    DrawShadowText(adFont, pcard->rkstring,
                   irr::core::recti(x2 + half_slash_width + slash.Width, y2,
                                    x2 - half_slash_width, y2 + 1),
                   padding_1111, skin::DUELFIELD_CARD_RANK_VAL, 0xff000000);
  } else if (pcard->rank != 0)
    DrawShadowText(adFont, pcard->rkstring,
                   irr::core::recti(x2, y2, x2 + 1, y2 + 1), padding_1011,
                   skin::DUELFIELD_CARD_RANK_VAL, 0xff000000);
  else if (pcard->level != 0)
    DrawShadowText(adFont, pcard->lvstring,
                   irr::core::recti(x2, y2, x2 + 1, y2 + 1), padding_1011,
                   GetLevelColor(), 0xff000000);
  else if (pcard->link != 0)
    DrawShadowText(adFont, pcard->linkstring,
                   irr::core::recti(x2, y2, x2 + 1, y2 + 1), padding_1011,
                   skin::DUELFIELD_CARD_LINK_VAL, 0xff000000);
}
/*
Draws the pendulum scale value of a card in the pendulum zone based on its
relative position
*/
void Game::DrawPendScale(ClientCard *pcard) {
  ...
  auto coords =
      device->getSceneManager()
          ...
          ->getScreenCoordinatesFrom3DPosition({x0, y0, pcard->curPos.Z});
  DrawShadowText(adFont, scale,
                 irr::core::recti(coords.X - (12 * swap), coords.Y,
                                  coords.X + (12 * (1 - swap)), coords.Y - 800),
                 Resize(1, 1, 1, 1), skin::DUELFIELD_CARD_PSCALE_VAL,
                 0xff000000, true);
}
/*
Draws the text in the middle of the bottom side of the zone
*/
void Game::DrawStackIndicator(epro::wstringview text,
                              const Materials::QuadVertex v, bool opponent) {
  ...
  auto coords = device->getSceneManager()
                    ->getSceneCollisionManager()
                    ->getScreenCoordinatesFrom3DPosition({x0, y0, 0});
  DrawShadowText(numFont, utext,
                 irr::core::recti(coords.X - dim.Width, coords.Y - dim.Height,
                                  coords.X + dim.Width, coords.Y + dim.Height),
...
void Game::DrawGUI() {
  if (imageLoading.size()) {
    for (auto mit = imageLoading.begin(); mit != imageLoading.end();) {
      int check = 0;
      mit->first->setImage(imageManager.GetTextureCard(
          mit->second, imgType::ART, false, false, &check));
      if (check != 2)
    ...
    if (fu.fadingFrame != 0.0f) {
      ...
      if (fu.isFadein) {
        if (fu.fadingFrame > (int)(5.0f * 1000.0f / 60.0f)) {
          ...
          fu.guiFading->setRelativePosition(
              irr::core::recti(fu.fadingUL, fu.fadingLR));
        } else {
      ...
      if (fu.wasEnabled) {
        ...
        if (prevfocused &&
            (prevfocused->getType() == irr::gui::EGUIET_EDIT_BOX))
  ...
  // Draw modular card with bilinear filtering (after GUI so it appears on top)
  if (modularCardTexture && wCardImg->isVisible()) {
    // Get the position where imgCard would be drawn
    auto cardRect = imgCard->getAbsolutePosition();
    DrawTextureBilinear(modularCardTexture, cardRect, true);
  }
}
void Game::DrawSpec() {
  const auto drawrect2 =
      ResizeWin(574, 150, 574 + CARD_IMG_WIDTH, 150 + CARD_IMG_HEIGHT);
  if (showcard) {
    switch (showcard) {
    case 1: {
      auto cardtxt = imageManager.GetTextureCard(showcardcode, imgType::ART);
      auto cardrect = irr::core::rect<irr::s32>(
          irr::core::vector2di(0, 0),
          irr::core::dimension2di(cardtxt->getOriginalSize()));
      driver->draw2DImage(cardtxt, drawrect2, cardrect);
      driver->draw2DImage(
          imageManager.tMask,
          ResizeWin(574, 150,
                    574 + (showcarddif > CARD_IMG_WIDTH ? CARD_IMG_WIDTH
                                                        : showcarddif),
                    404),
          Scale<irr::s32>(CARD_IMG_HEIGHT - showcarddif, 0,
                          CARD_IMG_HEIGHT - (showcarddif > CARD_IMG_WIDTH
                                                 ? showcarddif - CARD_IMG_WIDTH
                                                 : 0),
                          CARD_IMG_HEIGHT),
          0, 0, true);
    ...
    case 2: {
      ...
      driver->draw2DImage(cardtxt, drawrect2, cardrect);
      driver->draw2DImage(
          imageManager.tMask, ResizeWin(574 + showcarddif, 150, 751, 404),
          Scale(0, 0, CARD_IMG_WIDTH - showcarddif, 254), 0, 0, true);
      showcarddif += (900.0f / 1000.0f) * (float)delta_time;
    ...
    case 3: {
      ...
      driver->draw2DImage(imageManager.tNegated,
                          ResizeWin(536 + showcarddif, 141 + showcarddif,
                                    793 - showcarddif, 397 - showcarddif),
                          Scale(0, 0, 128, 128), 0, 0, true);
      if (showcarddif < 64)
    ...
    case 4: {
      ...
      driver->draw2DImage(cardtxt, drawrect2, cardrect, 0, matManager.c2d,
                          true);
      if (showcarddif < 255)
    ...
    case 5: {
      ...
      auto rect =
          ResizeWin(662 - showcarddif * (CARD_IMG_WIDTH_F / CARD_IMG_HEIGHT_F),
                    277 - showcarddif,
                    662 + showcarddif * (CARD_IMG_WIDTH_F / CARD_IMG_HEIGHT_F),
                    277 + showcarddif);
    ...
    case 6: {
      ...
      driver->draw2DImage(imageManager.tNumber,
                          ResizeWin(536 + showcarddif, 141 + showcarddif,
                                    793 - showcarddif, 397 - showcarddif),
                          Scale(((int)std::round(showcardp) % 5) * 64,
                                ((int)std::round(showcardp) / 5) * 64,
                                ((int)std::round(showcardp) % 5 + 1) * 64,
                                ((int)std::round(showcardp) / 5 + 1) * 64),
                          0, 0, true);
      if (showcarddif < 64)
        showcarddif += (240.0f / 1000.0f) * (float)delta_time;
    ...
    case 7: {
      ...
      auto a = ResizeWin(
          574 - (CARD_IMG_HEIGHT - y) * 0.3f, (150 + CARD_IMG_HEIGHT) - y,
          751 + (CARD_IMG_HEIGHT - y) * 0.3f, 150 + CARD_IMG_HEIGHT);
      auto b = ResizeWin(574, 150, 574 + CARD_IMG_WIDTH, 404);
      corner[0] = a.UpperLeftCorner;
      corner[1] =
          irr::core::vector2di{a.LowerRightCorner.X, a.UpperLeftCorner.Y};
      corner[2] =
          irr::core::vector2di{b.UpperLeftCorner.X, b.LowerRightCorner.Y};
    ...
    case 101: {
      ...
      if (showcardp < 10.0f) {
        ...
        DrawShadowText(lpcFont, lstr,
                       ResizePhaseHint(661 - (9 - showcardp) * 40, 291, 960,
                                       370, pos.Width),
                       Resize(-1, -1, 0, 0), alpha | 0xffffff, alpha);
      } else if (showcardp < showcarddif) {
        DrawShadowText(lpcFont, lstr,
                       ResizePhaseHint(661, 291, 960, 370, pos.Width),
                       Resize(-1, -1, 0, 0), 0xffffffff);
        if (dInfo.vic_string.size() &&
            (showcardcode == 1 || showcardcode == 2)) {
          auto a = (291 + pos.Height + 2);
          driver->draw2DRectangle(0xa0000000, Resize(540, a, 790, a + 20));
          DrawShadowText(guiFont, dInfo.vic_string,
                         Resize(492, a + 1, 840, a + 20), Resize(-2, -1, 0, 0),
                         0xffffffff, 0xff000000, true, true);
        }
      } else if (showcardp < showcarddif + 10.0f) {
        int alpha =
            (int)std::round(
                (((showcarddif + 10.0f - showcardp) * 25.0f) / 1000.0f) *
                (float)delta_time)
            << 24;
        DrawShadowText(lpcFont, lstr,
                       ResizePhaseHint(661 + (showcardp - showcarddif) * 40,
                                       291, 960, 370, pos.Width),
                       Resize(-1, -1, 0, 0), alpha | 0xffffff, alpha);
      }
      showcardp += std::min(((float)delta_time * 60.0f / 1000.0f),
                            showcarddif - showcardp);
      break;
    }
    }
  }
  if (is_attacking) {
    ...
    matManager.mATK.DiffuseColor =
        (skin::DUELFIELD_ATTACK_ARROW_VAL.getAlpha() << 24) | 0xffffff;
    driver->setMaterial(matManager.mATK);
    driver->drawVertexPrimitiveList(
        &matManager.vArrow[std::min(static_cast<int>(attack_sv) * 4, 28)], 12,
        matManager.iArrow, 10, irr::video::EVT_STANDARD,
  ...
  for (int i = 0; i < 8; ++i) {
    static constexpr uint32_t chatColor[] = {
        0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff,
        0xffffffff, 0xffffffff, 0xffffffff, 0xff8080ff, 0xffff4040,
        0xffff4040, 0xffff4040, 0xff40ff40, 0xff4040ff, 0xff40ffff,
        0xffff40ff, 0xffffff40, 0xffffffff, 0xff808080, 0xff404040};
    if (chatTiming[i] > 0.0f) {
      ...
      rectloc -=
          irr::core::vector2di(0, (i + 1) * chatrect.getHeight() + Scale(1));
      rectloc.LowerRightCorner.X = rectloc.UpperLeftCorner.X + w + Scale(4);
      auto msgloc = chatrect;
      msgloc -= irr::core::vector2di(Scale(-2),
                                     (i + 1) * chatrect.getHeight() + Scale(1));
      auto shadowloc = msgloc + irr::core::vector2di(1, 1);
      driver->draw2DRectangle(rectloc, 0xa0000000, 0xa0000000, 0xa0000000,
                              0xa0000000);
      textFont->drawustring(chatMsg[i], msgloc, 0xff000000, false, false);
      textFont->drawustring(chatMsg[i], shadowloc, chatColor[chatType[i]],
                            false, false);
    }
  }
}
void Game::DrawBackImage(irr::video::ITexture *texture, bool resized) {
  ...
  if (texture != prevbg) {
    ...
    bg_size = irr::core::recti(0, 0, texture->getOriginalSize().Width,
                               texture->getOriginalSize().Height);
    if (!gGameConfig->scale_background) {
      irr::core::rectf dest_sizef(0, 0, dest_size.getWidth(),
                                  dest_size.getHeight());
      irr::core::rectf bg_sizef(0, 0, bg_size.getWidth(), bg_size.getHeight());
      float width = ((bg_sizef.getWidth() / bg_sizef.getHeight()) *
                     dest_sizef.getHeight()) -
                    dest_sizef.getWidth();
      float height = ((bg_sizef.getHeight() / bg_sizef.getWidth()) *
                      dest_sizef.getWidth()) -
                     dest_sizef.getHeight();
      if (width > 0) {
        int off = std::ceil(width * 0.5f);
        dest_size = irr::core::recti(
            {-off, 0, dest_size.getWidth() + off, dest_size.getHeight()});
      } else if (height > 0) {
        int off = std::ceil(height * 0.5f);
        dest_size = irr::core::recti(
            {0, -off, dest_size.getWidth(), dest_size.getHeight() + off});
      }
    }
  }
...
void Game::ShowElement(irr::gui::IGUIElement *win, int autoframe) {
  ...
  for (auto fit = fadingList.begin(); fit != fadingList.end(); ++fit) {
    if (win == fit->guiFading) {
      if (win != wOptions) // the size of wOptions is always set by
                           // ClientField::ShowSelectOption before showing it
...
void Game::WaitFrameSignal(int frame, std::unique_lock<epro::mutex> &_lck) {
  signalFrame = (gGameConfig->quick_animation && frame >= 12)
                    ? 12 * 1000 / 60
                    : frame * 1000 / 60;
  frameSignal.Wait(_lck);
}
void Game::DrawThumb(const CardDataC *cp, irr::core::vector2di pos,
                     LFList *lflist, bool drag,
                     const irr::core::recti *cliprect, bool load_image) {
  auto code = cp->code;
  ...
  irr::video::ITexture *img =
      load_image ? imageManager.GetTextureCard(code, imgType::THUMB)
                 : imageManager.tUnknown;
  if (!img)
    return;
  irr::core::dimension2du size = img->getOriginalSize();
  irr::core::recti dragloc =
  ...
  irr::core::recti otloc =
      Resize(pos.X + 7, pos.Y + 50, pos.X + 37, pos.Y + 65);
  if (drag) {
    dragloc = irr::core::recti(
        pos.X, pos.Y, pos.X + Scale(CARD_THUMB_WIDTH * window_scale.X),
        pos.Y + Scale(CARD_THUMB_HEIGHT * window_scale.Y));
    limitloc =
        irr::core::recti(pos.X, pos.Y, pos.X + Scale(20 * window_scale.X),
                         pos.Y + Scale(20 * window_scale.Y));
    otloc = irr::core::recti(pos.X + 7, pos.Y + 50 * window_scale.Y,
                             pos.X + 37 * window_scale.X,
                             pos.Y + 65 * window_scale.Y);
  }
  driver->draw2DImage(
      img, dragloc, irr::core::recti(0, 0, size.Width, size.Height), cliprect);
  if (!is_siding) {
    switch (count) {
    case -1:
    case 0:
      imageManager.draw2DImageFilterScaled(imageManager.tLim, limitloc,
                                           irr::core::recti(0, 0, 64, 64),
                                           cliprect, 0, true);
      break;
    case 1:
      imageManager.draw2DImageFilterScaled(imageManager.tLim, limitloc,
                                           irr::core::recti(64, 0, 128, 64),
                                           cliprect, 0, true);
      break;
    case 2:
      imageManager.draw2DImageFilterScaled(imageManager.tLim, limitloc,
                                           irr::core::recti(0, 64, 64, 128),
                                           cliprect, 0, true);
      break;
    default:
      if (cp->ot & SCOPE_LEGEND) {
        imageManager.draw2DImageFilterScaled(imageManager.tLim, limitloc,
                                             irr::core::recti(64, 64, 128, 128),
                                             cliprect, 0, true);
      }
      break;
    }
#define IDX(scope, idx)                                                        \
  case SCOPE_##scope:                                                          \
    index = idx;                                                               \
    goto draw;
    if (gGameConfig->showScopeLabel && !lflist->whitelist) {
      // Label display logic:
      // If it contains exactly one bit between Anime, Illegal, Video Game,
      // Custom, and Prerelease, display that. Else, if it contains exactly one
      // bit between OCG and TCG, display that.
      ...
      default:
        break;
      draw:
        imageManager.draw2DImageFilterScaled(
            imageManager.tOT, otloc,
            irr::core::recti(0, index * 64, 128, index * 64 + 64), cliprect, 0,
            true);
      }
    }
#undef IDX
...
#define DECKCOLOR(what)                                                        \
  SKCOLOR(what##_TOP_LEFT), SKCOLOR(what##_TOP_RIGHT),                         \
      SKCOLOR(what##_BOTTOM_LEFT), SKCOLOR(what##_BOTTOM_RIGHT)
#define DRAWRECT(what, ...)                                                    \
  do {                                                                         \
    driver->draw2DRectangle(Resize(__VA_ARGS__), DECKCOLOR(what));             \
  } while (0)
#define DRAWOUTLINE(what, ...)                                                 \
  do {                                                                         \
    driver->draw2DRectangleOutline(Resize(__VA_ARGS__),                        \
                                   SKCOLOR(what##_OUTLINE));                   \
  } while (0)
void Game::DrawDeckBd() {
  const auto GetDeckSizeStr =
      [&](const Deck::Vector &deck,
          const Deck::Vector &pre_deck) -> std::wstring {
    if (is_siding)
  ...
  // main deck
  {
    ...
    DrawShadowText(textFont, gDataManager->GetSysString(1330),
                   Resize(314, 136, 409, 156), Resize(1, 1, 1, 1), 0xffffffff,
                   0xff000000, false, true);

    const auto main_deck_size_str =
        GetDeckSizeStr(current_deck.main, gdeckManager->pre_deck.main);
    DrawShadowText(numFont, main_deck_size_str, Resize(379, 137, 439, 157),
                   Resize(1, 1, 1, 1), 0xffffffff, 0xff000000, false, true);

    const auto main_types_count_str = epro::format(
        L"{} {} {} {} {} {}", gDataManager->GetSysString(1312),
        deckBuilder.main_monster_count, gDataManager->GetSysString(1313),
        deckBuilder.main_spell_count, gDataManager->GetSysString(1314),
        deckBuilder.main_trap_count);

    const auto mainpos = Resize(310, 137, 797, 157);
    const auto mainDeckTypeSize =
        textFont->getDimensionustring(main_types_count_str);
    const auto pos = irr::core::recti(
        mainpos.LowerRightCorner.X - mainDeckTypeSize.Width - 5,
        mainpos.UpperLeftCorner.Y, mainpos.LowerRightCorner.X,
        mainpos.LowerRightCorner.Y);

    DrawShadowText(textFont, main_types_count_str, pos,
                   irr::core::recti{1, 1, 1, 1}, 0xffffffff, 0xff000000, false,
                   true);
    ...
    const int cards_per_row =
        (current_deck.main.size() > 40)
            ? static_cast<int>((current_deck.main.size() - 41) / 4 + 11)
            : 10;
    const float dx = 436.0f / (cards_per_row - 1);

    for (int i = 0; i < static_cast<int>(current_deck.main.size()); ++i) {
      DrawThumb(current_deck.main[i],
                irr::core::vector2di(314 + (i % cards_per_row) * dx,
                                     164 + (i / cards_per_row) * 68),
                deckBuilder.filterList);
      if (deckBuilder.hovered_pos == 1 && deckBuilder.hovered_seq == i)
        driver->draw2DRectangleOutline(
            Resize(
                313 + (i % cards_per_row) * dx, 163 + (i / cards_per_row) * 68,
                359 + (i % cards_per_row) * dx, 228 + (i / cards_per_row) * 68),
            skin::DECK_WINDOW_HOVERED_CARD_OUTLINE_VAL);
    }
  }
  // extra deck
  {
    ...
    DrawShadowText(textFont, gDataManager->GetSysString(1331),
                   Resize(314, 439, 409, 459), Resize(1, 1, 1, 1), 0xffffffff,
                   0xff000000, false, true);

    const auto extra_deck_size_str =
        GetDeckSizeStr(current_deck.extra, gdeckManager->pre_deck.extra);
    DrawShadowText(numFont, extra_deck_size_str, Resize(379, 440, 439, 460),
                   Resize(1, 1, 1, 1), 0xffffffff, 0xff000000, false, true);

    const auto extra_types_count_str = epro::format(
        L"{} {} {} {} {} {} {} {} {} {}", gDataManager->GetSysString(1056),
        deckBuilder.extra_fusion_count, gDataManager->GetSysString(1073),
        deckBuilder.extra_xyz_count, gDataManager->GetSysString(1063),
        deckBuilder.extra_synchro_count, gDataManager->GetSysString(1076),
        deckBuilder.extra_link_count, gDataManager->GetSysString(1057),
        deckBuilder.extra_rush_ritual_count);

    const auto extrapos = Resize(310, 440, 797, 460);
    const auto extraDeckTypeSize =
        textFont->getDimensionustring(extra_types_count_str);
    const auto pos = irr::core::recti(
        extrapos.LowerRightCorner.X - extraDeckTypeSize.Width - 5,
        extrapos.UpperLeftCorner.Y, extrapos.LowerRightCorner.X,
        extrapos.LowerRightCorner.Y);

    DrawShadowText(textFont, extra_types_count_str, pos,
                   irr::core::recti{1, 1, 1, 1}, 0xffffffff, 0xff000000, false,
                   true);
    ...
    const float dx = (current_deck.extra.size() <= 10)
                         ? (436.0f / 9.0f)
                         : (436.0f / (current_deck.extra.size() - 1));

    for (size_t i = 0; i < current_deck.extra.size(); ++i) {
      DrawThumb(current_deck.extra[i], irr::core::vector2di(314 + i * dx, 466),
                deckBuilder.filterList);
  ...
  // side deck
  {
    ...
    DrawShadowText(textFont, gDataManager->GetSysString(1332),
                   Resize(314, 536, 409, 556), Resize(1, 1, 1, 1), 0xffffffff,
                   0xff000000, false, true);

    const auto side_deck_size_str =
        GetDeckSizeStr(current_deck.side, gdeckManager->pre_deck.side);
    DrawShadowText(numFont, side_deck_size_str, Resize(379, 536, 439, 556),
                   Resize(1, 1, 1, 1), 0xffffffff, 0xff000000, false, true);

    const auto side_types_count_str = epro::format(
        L"{} {} {} {} {} {}", gDataManager->GetSysString(1312),
        deckBuilder.side_monster_count, gDataManager->GetSysString(1313),
        deckBuilder.side_spell_count, gDataManager->GetSysString(1314),
        deckBuilder.side_trap_count);

    const auto sidepos = Resize(310, 537, 797, 557);
    const auto sideDeckTypeSize =
        textFont->getDimensionustring(side_types_count_str);
    const auto pos = irr::core::recti(
        sidepos.LowerRightCorner.X - sideDeckTypeSize.Width - 5,
        sidepos.UpperLeftCorner.Y, sidepos.LowerRightCorner.X,
        sidepos.LowerRightCorner.Y);

    DrawShadowText(textFont, side_types_count_str, pos,
                   irr::core::recti{1, 1, 1, 1}, 0xffffffff, 0xff000000, false,
                   true);
    DRAWRECT(SIDE, 310, 560, 797, 630);
    DRAWOUTLINE(SIDE, 309, 559, 797, 630);

    const float dx = (current_deck.side.size() <= 10)
                         ? (436.0f / 9.0f)
                         : (436.0f / (current_deck.side.size() - 1));

    for (size_t i = 0; i < current_deck.side.size(); ++i) {
      DrawThumb(current_deck.side[i], irr::core::vector2di(314 + i * dx, 564),
                deckBuilder.filterList);
  ...
  // search result
  {
    ...
    DrawShadowText(textFont, gDataManager->GetSysString(1333),
                   Resize(809, 136, 914, 156), Resize(1, 1, 1, 1), 0xffffffff,
                   0xff000000, false, true);

    const auto tmpstring = gDataManager->GetSysString(1333);
    const auto size =
        textFont->getDimensionustring(tmpstring).Width + ResizeX(5);
    const auto pos = irr::core::recti(ResizeX(809) + size, ResizeY(136),
                                      ResizeX(809) + size + 10, ResizeY(156));
    DrawShadowText(numFont, deckBuilder.result_string, pos, Resize(0, 1, 0, 1),
                   0xffffffff, 0xff000000, false, true);
    ...
    deckBuilder.scroll_pos =
        floor(scrFilter->getPos() / DECK_SEARCH_SCROLL_STEP);

    const bool draw_thumb = std::abs(prev_pos - deckBuilder.scroll_pos) <
                            (10.0f * 60.0f / 1000.0f) * delta_time;
    const int card_position = deckBuilder.scroll_pos;
    const int height_offset =
        (scrFilter->getPos() % DECK_SEARCH_SCROLL_STEP) * -1.f * 0.65f;
    const irr::core::recti rect = Resize(805, 160, 1020, 630);

    // loads the thumb of one card before and one after to make the scroll
    // smoother
    int i = (card_position > 0) ? -1 : 0;
    for (; i < 9 && (i + card_position) < (int)deckBuilder.results.size();
         ++i) {
      auto ptr = deckBuilder.results[i + card_position];
      if (deckBuilder.hovered_pos == 4 && deckBuilder.hovered_seq == i)
        driver->draw2DRectangle(skin::DECK_WINDOW_HOVERED_CARD_RESULT_VAL,
                                Resize(806, height_offset + 164 + i * 66, 1019,
                                       height_offset + 230 + i * 66),
                                &rect);
      DrawThumb(ptr, irr::core::vector2di(810, height_offset + 165 + i * 66),
                deckBuilder.filterList, false, &rect, draw_thumb);
      if (ptr->type & TYPE_MONSTER) {
        DrawShadowTextPos(textFont, gDataManager->GetName(ptr->code),
                          Resize(859, height_offset + 164 + i * 66, 955,
                                 height_offset + 185 + i * 66),
                          Resize(860, height_offset + 165 + i * 66, 955,
                                 height_offset + 185 + i * 66),
                          0xffffffff, 0xff000000, false, false, &rect);
        if (ptr->type & TYPE_LINK) {
          DrawShadowTextPos(
              textFont,
              epro::format(L"{}/{}",
                           gDataManager->FormatAttribute(ptr->attribute),
                           gDataManager->FormatRace(ptr->race)),
              Resize(859, height_offset + 186 + i * 66, 955,
                     height_offset + 207 + i * 66),
              Resize(860, height_offset + 187 + i * 66, 955,
                     height_offset + 207 + i * 66),
              0xffffffff, 0xff000000, false, false, &rect);
        } else {
          ...
          DrawShadowTextPos(
              textFont,
              epro::format(
                  L"{}/{} {}{}", gDataManager->FormatAttribute(ptr->attribute),
                  gDataManager->FormatRace(ptr->race), form, ptr->level),
              Resize(859, height_offset + 186 + i * 66, 955,
                     height_offset + 207 + i * 66),
              Resize(860, height_offset + 187 + i * 66, 955,
                     height_offset + 207 + i * 66),
        ...
        DrawShadowTextPos(textFont, gDataManager->GetName(ptr->code),
                          ...
                          Resize(860, height_offset + 165 + i * 66, 955,
                                 height_offset + 185 + i * 66),
                          0xffffffff, 0xff000000, false, false, &rect);
        DrawShadowTextPos(textFont, gDataManager->FormatType(ptr->type),
                          Resize(859, height_offset + 186 + i * 66, 955,
                                 height_offset + 207 + i * 66),
                          Resize(860, height_offset + 187 + i * 66, 955,
                                 height_offset + 207 + i * 66),
      ...
      DrawShadowTextPos(textFont, GetScopeString(),
                        Resize(859, height_offset + 208 + i * 66, 955,
                               height_offset + 229 + i * 66),
                        Resize(860, height_offset + 209 + i * 66, 955,
                               height_offset + 229 + i * 66),
                        0xffffffff, 0xff000000, false, false, &rect);
    }
  }
  if (deckBuilder.is_draging)
    DrawThumb(
        deckBuilder.dragging_pointer,
        irr::core::vector2di(deckBuilder.dragx - Scale(CARD_THUMB_WIDTH / 2),
                             deckBuilder.dragy - Scale(CARD_THUMB_HEIGHT / 2)),
        deckBuilder.filterList, true);
}
#undef DRAWRECT
#undef DECKCOLOR
#undef SKCOLOR
